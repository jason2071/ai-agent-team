...
- **Frontend-only iteration: run `npm run dev` (Vite alone)** instead of `npm run tauri dev` — instant reload, skips Rust compile entirely. Use the full `tauri dev` only when touching `src-tauri/`.
